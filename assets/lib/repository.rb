...
require_relative 'filters/branch_regex'

class Repository
  attr_reader :name

  def initialize(name:, input: Input.instance, filters: [Filters::All, Filters::Path, Filters::Fork, Filters::Label, Filters::CISkip, Filters::Mergeable, Filters::Approval, Filters::BranchRegex])
    @filters = filters
    @name    = name
    @input   = input
  end
